...
public class WeepingAngelEntity extends QuantumLockBaseEntity {
    ...
    private void replaceBlocks(AxisAlignedBB box) {
        if (ticksExisted % 100 != 0 || isQuantumLocked()) return;
        ...
        switch (type) {
            ...
            case RANDOM_PLACE:
                ...
                world.getServer().enqueue(new TickDelayedTask(0, () -> {
                    ...
                    BlockPos blockPos = new BlockPos(x, WATeleporter.yCoordSanity(teleportWorld, new BlockPos(x, 0, z)), z);

                    if (AngelUtils.isOutsideOfBorder(world, blockPos)) {
                        blockPos = world.getSpawnPoint().add(12, 0, 12);
                        blockPos = new BlockPos(blockPos.getX(), WATeleporter.yCoordSanity(world, blockPos), blockPos.getZ());
