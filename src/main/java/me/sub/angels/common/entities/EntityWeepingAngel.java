...
public class EntityWeepingAngel extends EntityQuantumLockBase
{
	...
	private SoundEvent[] seenSounds = new SoundEvent[] { WAObjects.Sounds.ANGEL_SEEN_1, WAObjects.Sounds.ANGEL_SEEN_2, WAObjects.Sounds.ANGEL_SEEN_3,
			WAObjects.Sounds.ANGEL_SEEN_4, WAObjects.Sounds.ANGEL_SEEN_5 };
	...
	public void readEntityFromNBT(NBTTagCompound compound)
	{
		...
		if (compound.hasKey(WAConstants.POSE))
			setPose(compound.getString(WAConstants.POSE));

		if (compound.hasKey(WAConstants.TYPE))
			setType(compound.getInteger(WAConstants.TYPE));

		if (compound.hasKey(WAConstants.ANGEL_CHILD))
	...
	private void teleportTowards(EntityLivingBase entity)
	{
		...
		if (p == null)
			return;
		if(p.getCurrentPathLength() > p.getCurrentPathIndex() + 1)
	...
	private void teleportPlayer(EntityPlayer player)
	{
		...
		WATeleporter
				.teleportDimEntity(player, player.getPosition().add(x, ws.getTopSolidOrLiquidBlock(new BlockPos(x, 0, z)).getY() - player.posY, z), dim, this);
	}
