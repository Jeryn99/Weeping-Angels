...
public class WAConfig {
    ...
    public WAConfig(ForgeConfigSpec.Builder builder) {
        ...
        builder.pop();
        builder.push("spawn");
        minCount = builder.translation("config.weeping_angels.minCount").worldRestart().comment("The minimum amount of 'Weeping Angels' that spawn at each spawn attempt").defineInRange("minCount", 1, 1, 100);
        maxCount = builder.translation("config.weeping_angels.maxCount").worldRestart().comment("The maximum amount of 'Weeping Angels' that spawn at each spawn attempt").defineInRange("maxCount", 3, 1, 100);
